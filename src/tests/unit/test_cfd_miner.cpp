...
TEST(CFDMinerTest, MinesDependenciesWithTwoItemLhs) {
    CSVConfig const csv_config{kTennis.path.parent_path() / "cfdminer_lite.csv", ',', true};
    auto algorithm = CreateAlgorithm(csv_config, 2, 2);
    algorithm->Execute();

    auto const cfds = ToStrings(algorithm->GetCfds());
    EXPECT_TRUE(cfds.contains("{(0, x),(1, p)} -> (2, yes)"));
    EXPECT_TRUE(cfds.contains("{(0, y),(1, q)} -> (2, yes)"));
}
